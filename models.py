...
def encoding_media_file_path(instance, filename):
    ...
    print(settings.MEDIA_ENCODING_DIR + "{0}/{1}/{2}".format(instance.profile.id, instance.media.user.username, file_name))
...
class Media(models.Model):
    ...
    # Extracting the name from the page title
        page_title = self.title  # Replace with actual page title
    ...
    # Fetching player data using the access token
        ...
        json_data = response.json()
        new_id_array = [player['id'] for player in json_data.get('players', [])]
        new_id=new_id_array[0]
        hls_file_url = f'https://video.platformcraft.ru/vod/{new_id}/playlist.m3u8'
        ...
        if hls_file_url:
        # Load the HLS file from the CDN
            m3u8_obj = m3u8.load(hls_file_url)
        
        # Add the master file URL to the result dictionary
            res["master_file"] = hls_file_url
        
        # Process iframe playlists
            for iframe_playlist in m3u8_obj.iframe_playlists:
                uri = iframe_playlist.uri
            ...
            # Add the iframe playlist URL to the result dictionary
                res["{}_iframe".format(resolution)] = uri
        ...
        # Process regular playlists
            ...
            # Add the regular playlist URL to the result dictionary
                res["{}_playlist".format(resolution)] = uri
...
class Stream(models.Model):
    date_produced = models.DateTimeField("Date produced", blank=True, null=True, db_index=True)
    category = models.ManyToManyField("Category", blank=True, help_text="Media can be part of one or more categories")
    channel = models.ForeignKey(
        "users.Channel",
        on_delete=models.CASCADE,
    ...
    # Additional fields for Stream class
    stream_url = models.CharField(max_length=1000, blank=True, help_text="URL for streaming the media")
    resolution = models.CharField(max_length=50, blank=True, help_text="Resolution of the media stream")
    encoding_format = models.CharField(max_length=50, blank=True, help_text="Encoding format of the media stream")
    is_live = models.BooleanField(default=False, help_text="Whether the stream is live or recorded")
    stream_duration = models.IntegerField(default=0, help_text="Duration of the media stream")

    def get_info_from_CDN():
        url = 'https://auth.platformcraft.ru/token'
        headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        data = {'login': 'greenatom', 'password': '123123'} 
        # Sending a POST request to retrieve the access token
    ...
    def get_title(self):
        data = self.get_info_from_CDN()
        if data:
            # Use the 'data' variable here to get the title
            title = [video.get('name') for video in data.get('contents')]
            return title
        else:
            return None
    ...
    def produce_thumbnails_from_video(self):
        ...
        Only for video types. Uses ffmpeg
        """
        # Set the static image path for thumbnails
        static_image_path = os.path.join(settings.STATIC_ROOT, "images", "thumbnail.jpg")
    ...
    @property
    def get_encodings_info(stream, full=False, data=None):
        ret = {}

        if stream.media_type not in ["video"]:
            return ret
        
        for key in ENCODE_RESOLUTIONS_KEYS:
            ret[key] = {}
        
        for encoding in stream.encodings.select_related("profile").filter(chunk=False):
            if encoding.profile.extension == "gif":
                continue
            enc = get_encoding_info(encoding, full=full, data=data)
            resolution = encoding.profile.resolution
            ret[resolution][encoding.profile.codec] = enc

        if full:
            extra = []
            for encoding in stream.encodings.select_related("profile").filter(chunk=True):
                resolution = encoding.profile.resolution
                if not ret[resolution].get(encoding.profile.codec):
                    extra.append(encoding.profile.codec)
            for codec in extra:
                ret[resolution][codec] = {}
                v = stream.encodings.filter(chunk=True, profile__codec=codec).values("progress")
                ret[resolution][codec]["progress"] = sum([p["progress"] for p in v]) / v.count()
        
        return ret

    def get_encoding_info(encoding, full=False, data=None):
        ep = {}
        ep["title"] = encoding.profile.name
        ep["url"] = encoding.media_encoding_url
        ep["progress"] = encoding.progress
        ...
        if data:
            # Extract encoding-specific information from the data
            resolution = encoding.profile.resolution
            codec = encoding.profile.codec

            for content in data.get("contents", []):
                for stream in content.get("advanced", {}).get("video_streams", []):
                    if stream.get("width") == resolution[0] and stream.get("height") == resolution[1]:
                        ep["bit_rate"] = stream.get("bit_rate")
                        ep["codec_type"] = stream.get("codec_type")
                        ep["duration"] = stream.get("duration")
                        break

                for stream in content.get("advanced", {}).get("audio_streams", []):
                    if stream.get("codec_name") == codec:
                        ep["audio_bit_rate"] = stream.get("bit_rate")
                        ep["audio_sample_rate"] = stream.get("sample_rate")
                        break
    ...
    def get_original_media_url(self):
        if settings.SHOW_ORIGINAL_MEDIA:
    ...
    def get_absolute_url(self, api=False, edit=False):
        if edit:
            return reverse("edit_stream") + "?s={0}".format(self.friendly_token)
        if api:
            return reverse("api_get_stream", kwargs={"friendly_token": self.friendly_token})
        else:
            return reverse("get_stream") + "?s={0}".format(self.friendly_token)
    @property
    def edit_url(self):
        return self.get_absolute_url(edit=True)
