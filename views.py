...
class StreamListView(View):
    def get(self, request):
        media_data = self.retrieve_media_data()
        
        thumbnails = media_data.get("thumbnails", [])
        names = media_data.get("names", [])
        source_links = media_data.get("source_links", [])
    ...
    def retrieve_media_data(self):
        ...
        if response.status_code == 200:
            ...
            thumbnails = ['https://' + video.get('preview_url') if video.get('preview_url') else 'static/images/prevpic.png' for video in data.get('contents')]
            source_links = ["https://" + link.get('download_url') for link in data.get('contents')]
    ...
    def open_stream_page(self, request, source_link):
        context = {
            "source_link": source_link,
        }
        return render(request, "stream_page.html", context)


def stream_view(request):
    """Stream view"""

    friendly_token = request.GET.get("s", "").strip()
    context = {}
    stream = Stream.objects.filter(friendly_token=friendly_token).first()
    print(stream)
    if not stream:
        context["stream"] = None
        return render(request, "cms/stream.html", context)

    user_or_session = get_user_or_session(request)
    save_user_action.delay(user_or_session, friendly_token=friendly_token, action="watch")
    context = {}
    context["stream"] = friendly_token
    context["stream_object"] = stream

    context["CAN_DELETE_STREAM"] = False
    context["CAN_EDIT_STREAM"] = False
    context["CAN_DELETE_COMMENTS"] = False

    if request.user.is_authenticated:
        if (stream.user.id == request.user.id) or is_mediacms_editor(request.user) or is_mediacms_manager(request.user):
            context["CAN_DELETE_STREAM"] = True
            context["CAN_EDIT_STREAM"] = True
            context["CAN_DELETE_COMMENTS"] = True
